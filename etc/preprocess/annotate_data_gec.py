...
class ErrorAnnotation:
    ...
    def annotate_error(self, correct, error):
        ...
        self.correction = []
        self.error_label = ['O'] * len(self.annotation)
                                        ...
                                        if data[0] == ' ':
                                            ...
                                            if diff[idx - 2][1][-1] == ' ':
                                                if data[0] == ' ':
                                                    ...
                                                    error_text = diff[idx - 3][1].split()[-1] + diff[idx - 1][1] + data.split()[0]
                                                    correct_text = diff[idx - 3][1].split()[-1] + diff[idx - 2][1] + data.split()[0]
                                                    self.annotate_label_idx(error_text, correct_text, error, correct)
                                            # 교체 오류
        ...
        while e_flag:
            try:
                s_ei, e_ei = e_pattern.search(''.join(self.error_label)).span()
                s_ci, e_ci = c_pattern.search(''.join(self.correct_label)).span()
            except:
                return [], []
            e_word = error[s_ei:e_ei]
            c_word = correct[s_ci:e_ci]
            self.annotation[s_ei + adjust_len:e_ei + adjust_len] = f'<S>{e_word}</S>'
            error_sentence_lst.append(''.join(self.annotation))
            correct_word_lst.append(c_word)
            next_sentence = ''.join(self.annotation).replace(f'<S>{e_word}</S>', f'{c_word}')
            self.annotation = [i for i in next_sentence]
            self.error_label[s_ei:e_ei] = ['O'] * (e_ei - s_ei)
            self.correct_label[s_ci:e_ci] = ['O'] * (e_ci - s_ci)
            adjust_len += len(c_word) - len(e_word)
            if 'E' not in self.error_label and 'C' not in self.correct_label:
                e_flag = False

        return error_sentence_lst, correct_word_lst
...
if __name__ == '__main__':
    ...
    for i, line in tqdm(enumerate(lines)):
        ...
        if '\t' in line:
            ...
            error_sentence_lst, correct_word_lst  = annotation_tool.annotate_error(c, e)
            if not error_sentence_lst:
                continue

            idx_lst = [i] * len(correct_word_lst)

            with open('output_data/output_gec_sample.jsonl', 'w', encoding='utf-8') as wf:
                for idx, error_sentence, correct_word in zip(idx_lst, error_sentence_lst, correct_word_lst):
                    example = {"error": error_sentence, "correct": correct_word}
